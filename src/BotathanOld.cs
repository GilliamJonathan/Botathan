...
class Botathan
{
    ...
    public void VoiceModule()
    {
        cService.CreateCommand("soundboard")
        .Alias(new String[] { "soundboard", "sb" })
        .Description("Joins your current channel and plays a sound off the soundboard")
        .Parameter("sound", ParameterType.Required)
        .Do(async e =>
        {
            if(e.GetArg("sound").Equals("list"))
            {
                await e.Channel.SendMessage("I haven't writen them all down yet");
            }
            else
            {
                Console.WriteLine("Connecting");
                _vClient = await _client.GetService<AudioService>().Join(_client.FindServers("tester").FirstOrDefault().VoiceChannels.FirstOrDefault());
                Console.WriteLine("Conected!");
                SendAudio(@"C:\Users\gilli\Desktop\Botathan\src\audio\" + e.GetArg("sound") + ".mp3");
                _vClient.Wait();
